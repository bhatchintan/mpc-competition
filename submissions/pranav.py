...
CONTROL_GRID = {
    ...
    "dynamic_cruise_speeds": [0.5, 0.9, 1.3, 1.8, 2.3],
...
class Agent:
    ...
    def act(self, observation: dict[str, Any]) -> list[float]:
        ...
        for candidate_control in candidate_controls(state, goal, control_limits, scene):
...
def candidate_controls(
    ...
    if scene == "dynamic":
        speeds = CONTROL_GRID["dynamic_cruise_speeds"]
    ...
    turn_rates.add(round(proportional_turn + 0.35, 3))
    turn_rates.add(round(proportional_turn - 0.15, 3))
    turn_rates.add(round(proportional_turn + 0.15, 3))
...
def evaluate_rollout(
    ...
    total_cost = 0.0
    obstacle_cost_scale = 1.4 if scene == "dynamic" else 1.0
    ...
    for step_index in range(1, int(MPC_CONFIG["horizon"]) + 1):
        ...
        if step_index == 1:
            total_cost += WEIGHTS["smoothness"] * float(
                np.linalg.norm(predicted_control - previous_control)
            )
        ...
        for obstacle in active_obstacles:
            ...
            total_cost += obstacle_cost_scale * collision_cost(clearance)
