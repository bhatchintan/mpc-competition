...
CONTROL_GRID = {
    ...
    "cruise_speeds": [0.5, 0.9, 1.3700361376803185, 2.024543003655771, 2.716898432499149, 3.4402471014328118],
...
class Agent:
    ...
    def act(self, observation: dict[str, Any]) -> list[float]:
        ...
        for candidate_control in candidate_controls(
            state, goal, control_limits, active_obstacles, robot_radius
        ):
...
def candidate_controls(
    ...
    speeds = list(CONTROL_GRID["cruise_speeds"])
    if distance_to_goal < float(MPC_CONFIG["near_goal_distance"]):
        # Close to a waypoint, slower controls reduce overshoot.
        speeds = list(CONTROL_GRID["slow_speeds"])
    else:
        # mysense obstacle detection & slowing down
        obstacle_close = False
        for obstacle in active_obstacles:
            clearance = rectangle_clearance(state, obstacle, robot_radius)
            if clearance < 1.5:
                obstacle_close = True
                break
        if obstacle_close:
            speeds.extend([0.0, 0.3])
